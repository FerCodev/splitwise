...
<div class="container py-4 friends-page">
        ...
        <?php if ($searched): ?>
            ...
            <?php if ($found): ?>
                <div class="friend-row">
                    ...
                    <div><strong class="d-block"><?= esc($found['name']) ?></strong><small class="text-muted">@<?= esc($found['username']) ?></small></div>
                    <div class="ms-auto">
            ...
            <?php else: ?><p class="text-muted mb-0">No encontramos un usuario con ese email o nombre de usuario exacto.</p><?php endif; ?>
            </div>
        <?php endif; ?>
    </div></div>
    ...
    <?php if ($received): ?><h5>Solicitudes recibidas</h5><div class="card mb-4"><div class="card-body p-0">
        <?php foreach ($received as $item): ?><div class="friend-row">
            <?= view('components/avatar', ['userId' => $item['other_id'], 'name' => $item['other_name'], 'avatarFilename' => $item['other_avatar_filename'], 'avatarUpdatedAt' => $item['other_avatar_updated_at'], 'size' => 44]) ?><div><strong class="d-block"><?= esc($item['other_name']) ?></strong><small class="text-muted">@<?= esc($item['other_username']) ?></small></div>
    ...
    <div class="card"><div class="card-body p-0">
        ...
        <?php foreach ($friends as $item): ?><div class="friend-row">
            ...
            <a href="<?= base_url('usuarios/'.$item['other_id'].'/perfil') ?>" class="text-decoration-none flex-grow-1"><strong class="d-block"><?= esc($item['other_name']) ?></strong><small class="text-muted">@<?= esc($item['other_username']) ?></small></a>
