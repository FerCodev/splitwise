...
<div class="container mt-3 mt-md-4">
    ...
    <div class="row g-3 mb-3">
        <div class="col-12 col-lg-6"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center"><h5 class="mb-0 fw-bold">Gasto por grupo</h5><span class="text-muted small"><?= (int) ($resumenMensual['grupos_activos'] ?? 0) ?> activo(s)</span></div><div class="card-body p-0"><?php if (empty($porGrupo)): ?><p class="text-muted small p-3 mb-0">Sin gastos para el periodo.</p><?php else: ?><?php $maxGrupo = max(array_map(static fn($g) => (float) $g['total'], $porGrupo)); ?><?php foreach ($porGrupo as $g): ?><div class="mobile-card-item"><div class="d-flex justify-content-between align-items-center gap-2"><div class="fw-medium"><?= esc($g['grupo']) ?> <span class="text-muted small"><?= $g['cantidad'] ?> gasto(s)</span></div><div class="fw-bold text-primary"><?= moneda($g['total']) ?></div></div><div class="progress mt-2" style="height:4px"><div class="progress-bar bg-primary" style="width:<?= $maxGrupo > 0 ? ($g['total'] / $maxGrupo * 100) : 0 ?>%"></div></div></div><?php endforeach; ?><?php endif; ?></div></div></div>
        <div class="col-12 col-lg-6"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white border-bottom"><h5 class="mb-0 fw-bold">Gasto por categoria</h5></div><div class="card-body p-0"><?php if (empty($porCategoria)): ?><p class="text-muted small p-3 mb-0">Sin categorias para el periodo.</p><?php else: ?><?php $maxCategoria = max(array_map(static fn($c) => (float) $c['total'], $porCategoria)); ?><?php foreach ($porCategoria as $c): ?><div class="mobile-card-item"><div class="d-flex justify-content-between align-items-center gap-2"><div><span class="badge bg-light text-dark"><?= esc($c['categoria']) ?></span> <span class="text-muted small"><?= $c['cantidad'] ?> gasto(s)</span></div><div class="fw-bold"><?= moneda($c['total']) ?></div></div><div class="progress mt-2" style="height:4px"><div class="progress-bar bg-warning" style="width:<?= $maxCategoria > 0 ? ($c['total'] / $maxCategoria * 100) : 0 ?>%"></div></div></div><?php endforeach; ?><?php endif; ?></div></div></div>
    </div>

    <div class="row g-3 mb-4">
        <div class="col-12 col-lg-7"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white border-bottom"><h5 class="mb-0 fw-bold">Movimientos del periodo</h5></div><div class="card-body report-movement-list"><?php if (empty($movimientos)): ?><p class="text-muted small p-3 mb-0">No hay movimientos para el periodo.</p><?php else: ?><?php foreach ($movimientos as $m): ?><a href="<?= base_url(($m['tipo'] === 'gasto' ? 'gastos/' : 'pagos/') . $m['id']) ?>" class="report-movement-link"><div class="report-movement-card report-movement-<?= $m['tipo'] === 'gasto' ? 'expense' : 'payment' ?>" style="border-left: 3px solid <?= $m['tipo'] === 'gasto' ? '#2563eb' : '#16a34a' ?>;"><div class="d-flex justify-content-between gap-2"><div><span class="badge <?= $m['tipo'] === 'gasto' ? 'bg-primary' : 'bg-success' ?>"><?= $m['tipo'] === 'gasto' ? 'Gasto' : 'Pago' ?></span><span class="fw-medium small ms-1"><?= esc(mb_substr($m['descripcion'], 0, 40)) ?></span></div><span class="fw-bold small <?= $m['tipo'] === 'gasto' ? 'text-primary' : 'text-success' ?>"><?= moneda($m['monto']) ?></span></div><div class="text-muted small"><?= date('d/m/Y', strtotime($m['fecha'])) ?> &middot; <?= esc($m['persona']) ?> &middot; <?= esc($m['grupo']) ?></div></div></a><?php endforeach; ?><?php endif; ?></div></div></div>
        <div class="col-12 col-lg-5"><div class="card border-0 shadow-sm h-100"><div class="card-header bg-white border-bottom"><h5 class="mb-0 fw-bold">Saldos actuales</h5></div><div class="card-body p-0"><?php if (empty($deudas)): ?><p class="text-muted small p-3 mb-0">No hay deudas pendientes.</p><?php else: ?><?php foreach ($deudas as $d): ?><div class="report-debt-item" style="padding:14px 16px;border-left:3px solid #dc2626;border-bottom:1px solid #dee2e6;background:#fff;"><div class="d-flex justify-content-between align-items-center gap-2"><div><strong><?= esc($d['deudor']) ?></strong><span class="text-muted"> le debe a </span><strong><?= esc($d['acreedor']) ?></strong><?php if (!empty($d['grupo'])): ?><div class="text-muted small"><?= esc($d['grupo']) ?></div><?php endif; ?></div><div class="fw-bold text-danger"><?= moneda($d['monto']) ?></div></div></div><?php endforeach; ?><?php endif; ?></div></div></div>
    </div>
</div>
